...
function drawTwo() {
  ...
  fetch(url)
    ...
    .then((data) => {
      console.log(data);
      document.querySelector("#player1War").classList.add("hidden");
      document.querySelector("#player2War").classList.add("hidden");
      ...
      if (player1Val > player2Val) {
        ...
        dealCardButton.classList.add("hidden");
...
function war() {
  ...
  fetch(url)
    ...
    .then((data) => {
      console.log(data);
      dealCardButton.classList.remove("hidden");
      document.querySelector("#war").classList.add("hidden");

      // Display the card
      document.querySelector("#player1War").classList.remove("hidden");
      document.querySelector("#player2War").classList.remove("hidden");
      ...
      if (player1Val > player2Val) {
        displayRoundResult.innerText = "Player 1 Wins";
        displayScore.innerText = `P1: ${
          player1Score > 0 ? (player1Score *= 2) : (player1Score += 1)
        } | P2: ${player2Score}`;
        document.querySelector("#war").classList.add("hidden");
      } else if (player1Val < player2Val) {
        displayRoundResult.innerText = "Player 2 Wins";
        displayScore.innerText = `P1: ${player1Score} | P2: ${
          player2Score > 0 ? (player2Score *= 2) : (player2Score += 1)
        }`;
        document.querySelector("#war").classList.add("hidden");
      }
